<?php
foreach ($users as $users_item_key => $users_item_content)
{
	if (TRUE)
	{
		?>
			...
			<div class="users-item-content">
				...
				<div class="user-follow-actions">
					...
					if ($users_item_content['user_id'] <> $user_info['user_id'])
					{
						?>
						<button 
						class="btn btn-<?php echo (in_array($users_item_content['user_id'], $my_following_users)) ? 'info' : 'success';?> btn-xs bold font-12 link-actions-user" 
						data-user-id="<?php echo $users_item_content['user_id']; ?>" 
					...
					else
					{
						?>
						<button 
						class="btn btn-success btn-xs bold font-12 disabled" 
						>
							<span>It's you</span>
						</button>
						<?php
