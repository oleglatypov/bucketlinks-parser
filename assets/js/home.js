	...
	$(window).scroll(function ()
	{
		...
		if ($(window).scrollTop() + $(window).height() > $(document).height() - limit_to_bottom)
		{
			if ( ! bucketlinks.is_loading)
			{
				...
				var value__is_home_by_interest = $('#value__is_home_by_interest').val();

				$.ajax(
				{
					url: bucketlinks.site_url + 'ajax/post/get_home_posts',
					type: 'POST',
					data: { 'is_by_interest' : value__is_home_by_interest },
					dataType: 'json'
				})
